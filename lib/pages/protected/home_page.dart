...
class _HomePageState extends State<HomePage>
    with RefreshListenersMixin<HomePage> {
  late FitropeUser user;
  List<Course> allCourses = [];
  List<FitropeUser> trainers = [];

  /// Segnale verso `AdminHomeSections` dopo un'iscrizione o disiscrizione
  /// fatta dall'admin dalla Home: rilegge certificati e abbonamenti.
  final ValueNotifier<int> _adminRefresh = ValueNotifier(0);
  ...
  void initState() {
    ...
    // Le sezioni admin (con i loro listener) vivono in AdminHomeSections.
    // Il mixin toglie in dispose quello che si registra qui, anche se nel
    // frattempo `refreshCourses` ha cambiato `user` copiando lo store.
    if (user.role == 'User') {
      listenToRefresh(refreshCourses);
    }
    super.initState();
  }
  ...
  Future<void> refreshCourses() async {
    ...
    // Ricarica anche certificati e abbonamenti in scadenza se è montata la
    // sezione admin. Non si aspetta: quelle liste non decidono lo stato dei
    // pulsanti dei corsi.
    if (mounted) _adminRefresh.value++;
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          // SEZIONI ADMIN: certificati, abbonamenti, lezioni di prova e
          // regolamento. Libreria deferred: un socio non la scarica.
          if (user.role == 'Admin')
            DeferredSection(
              load: admin_home.loadLibrary,
              builder: (_) => admin_home.AdminHomeSections(
                allCourses: allCourses,
                refreshSignal: _adminRefresh,
              ),
            ),
        ],
      ),
