...
- **Anche l'emulatore locale carica `functions/.env.fit-rope-app-1f575`**, perché parte con `--project fit-rope-app-1f575`: lì la modalità è `test` e `sendTestDemoLessonWebhook` esiste. Per provarla in locale i secret Make vanno in `functions/.secret.local` (gitignored). I test di integrazione girano invece su `demo-fitrope`, in modalità `off`.
