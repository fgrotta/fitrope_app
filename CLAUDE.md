...
- **Brotli precompresso su Hostinger** (`tool/hostinger/README.md`): la brotli dinamica di LiteSpeed è a livello 6 sui file piccoli ma crolla su quelli grandi, come se comprimesse a blocchi indipendenti da ~2 KB (il `main.dart.js` di produzione, 4,9 MB, arrivava a 1,65 MB: peggio del gzip dello stesso server). Dopo il build `python3 tool/precompress_web.py build/web` crea i `.br` a livello 11 e `web/.htaccess` li serve a chi accetta brotli (rewrite + `ForceType` + `Content-Encoding: br` + `Vary`). Due vincoli: le estensioni della `RewriteRule` coincidono con `EXTENSIONS` dello script (test in `tool/hostinger/test_precompress_web.py`), e ogni `FilesMatch` di cache deve accettare anche il suffisso `(\.br)?`, perché dopo il rewrite Apache/LiteSpeed vede il nome del `.br`. La CI prova la `.htaccess` su un Apache vero (`tool/hostinger/local_apache_test.sh`): una `.htaccess` rotta su Hostinger dà 500 su tutto il sito. La versione precedente è in `tool/hostinger/htaccess.backup`, **fuori da `web/`** apposta (tutto ciò che sta in `web/` finisce online). Prima del primo deploy si fa la prova isolata con `make_brotli_probe.py` + `verify_brotli.sh`.
