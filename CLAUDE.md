...
- **`dart format` e versione dell'SDK**: il codice committato è formattato con un formatter più vecchio dello stile "tall" introdotto da Dart 3.9+. Su un SDK recente (qui 3.11) `dart format .` riformatta **tutto il repo** (~70 file, migliaia di righe) e affoga il diff reale. Formatta solo i file che stai toccando, controlla `git diff --stat` prima di committare e ripristina (`git checkout --`) qualunque file che compaia solo per la riformattazione.
...
- In staging (`APP_ENV=staging`) la Function invia **solo** a UID con prefisso `stg_`; per le email serve in più la allowlist `STAGING_NOTIFICATION_EMAIL_ALLOWLIST` (è un vincolo da inbox e non si applica alle push). Titolo e corpo sono marcati `[STAGING]` — `headings` compreso, non solo `contents`.
- **Guardrail push staging**: se `APP_ENV=staging` e l'app OneSignal di destinazione è quella di produzione, la push viene soppressa con `logger.error` (le email restano funzionanti). Lo stesso assert è in `staging.yml`, così il deploy fallisce prima del runtime.
- **`external_id` senza subscription push è lo stato NORMALE** di gran parte dei soci: il token nasce sul device e non esiste un equivalente server-side di `ensureOneSignalEmailSubscription`. OneSignal risponde 400 "All included players are not subscribed": `postToOneSignal` lo declassa a `logger.info` + `{ skipped: true }` invece di lanciare, e `recipients: 0` sul canale push è `info` (sull'email resta `warn`). La copertura push reale si misura da questi log.
...
- **Conditional import: `dart.library.js_interop`, MAI `dart.library.html`.** Sotto `--wasm` (come si builda la produzione e come è buildata staging) `dart:html` non esiste e `dart.library.html` è **false**: la direttiva classica sceglie il ramo non-web proprio sul web. È già successo a `onesignal_service.dart`, che così compilava il ramo *mobile* nel bundle web — raffica di `MissingPluginException`, nessun service worker registrato, né push né alias email. Il bug è invisibile in `flutter run -d chrome`, che compila in dart2js. Due gate: `test/conditional_imports_test.dart` (lint statico su tutti i conditional import/export di `lib/`) e un `grep -aq 'OneSignal Web' build/web/main.dart.wasm` dopo il build in `ci.yml` e `staging.yml`.
- **Il bridge JS ritorna Promise, non è fire-and-forget.** Ogni `window.oneSignal*` restituisce una Promise che risolve *sempre* (un reject attraverserebbe `JSPromise.toDart` come eccezione Dart), con timeout di sicurezza se il CDN non carica. **Il permesso push va chiesto dentro il gesto utente**: `oneSignalRequestPushPermission()` chiama `requestPermission()` in modo sincrono rispetto al tap, senza passare da `withOneSignal` — su WebKit la transient activation si perde al primo await e il prompt iOS non compare. Chi aggiunge un percorso di attivazione lo chiami come **prima istruzione** dell'`onPressed`. Da console: `await oneSignalDiagnostics()` dice se il device è davvero iscritto e con quale scope è registrato il worker.
- **Il diniego del browser non si scrive su Firestore.** `pushNotificationsEnabled` è un flag **cross-device**: un rifiuto su Chrome desktop non deve spegnere le push sull'iPhone. Nessun percorso deve scrivere `false` per aver letto il permesso locale.
- **Banner "attiva le notifiche"** (`lib/components/push_enable_banner.dart`): montato dentro `Protected`, non nel `builder` di `MaterialApp` (lì coprirebbe anche login e route pushate). La logica è la funzione pura `decidePushPrompt` in `lib/services/push_environment.dart` (`test/push_prompt_decision_test.dart`); lo snooze sta in `localStorage` via il trittico `lib/utils/push_prompt_prefs*.dart`. In simulazione non compare mai.
- **appId OneSignal del client via `--dart-define=ONESIGNAL_APP_ID`** (`lib/main.dart`): prod senza define usa l'app di produzione, staging senza define ottiene una stringa vuota e **salta l'init** con un warning — mai contaminazione. Client e Functions dello stesso ambiente devono puntare alla stessa app, o ogni invio finisce con `recipients: 0`.
...
- **Debug email**: in `kDebugMode` è disponibile un FAB in `Protected` che apre `DebugEmailPage` (`/debug-email`). Permette di inviare email di test (waitlist, conferma iscrizione prova, promemoria prova, certificato a 10 giorni / scadenza oggi) e una **push di test** all'utente corrente senza triggering reale degli eventi. La push è l'unico modo di provare una push su staging senza aspettare il cron delle 08:00. Le funzioni `sendTestWaitlistEmail` / `sendTestTrialConfirmationEmail` / `sendTestTrialReminderEmail` / `sendTestCertificateEmail` sono in `notification_service.dart`. I bottoni calendario delle email di test usano un evento demo (domani 10:00-11:00): i campi "Data"/"Orario" della pagina sono testo libero e non sono parsabili.
...
- **Setup esterno delle push staging (manuale, non fatto dal repo)**: serve una **seconda app OneSignal** per staging — Site URL `https://fgrotta.github.io`, integrazione **"Custom Code"** (con "typical"/"wordpress" l'SDK v16 ignora `serviceWorkerPath`/`serviceWorkerParam` passati a `init()` e cerca `https://fgrotta.github.io/OneSignalSDKWorker.js`, che è 404), prompt statici disabilitati e `welcomeNotification` disattivata. Poi: `firebase functions:secrets:set ONESIGNAL_REST_API_KEY --project staging` con la key della nuova app, e nell'environment GitHub `staging` le vars `ONESIGNAL_APP_ID` (nuovo valore) e `STAGING_APP_BASE_URL` = `https://fgrotta.github.io/fitrope_app/`. **App id e REST key devono atterrare nello stesso deploy**, altrimenti si apre una finestra di 401 su ogni invio, email comprese. Finché non è fatto, staging builda con appId vuoto (init saltata) e il deploy Functions fallisce sull'assert dell'app id di produzione.
