...
- **Build dart2js, non `--wasm`** (dal 26/09/2026): con Flutter 3.41.6 `deferred as` **non splitta il modulo wasm** (dart2wasm ha `--enable-deferred-loading`, ma `flutter_tools` non copia i moduli secondari in `build/web` e il loader dell'engine non passa `loadDeferredModule`), quindi il codice admin finiva nel bundle di tutti. Con dart2js `Protected` e il codice solo admin (`AdminUsersPage`, `AdminDashboardPage`, `UserListDrawer`, `AdminHomeSections`) sono part separati; i gate in `ci.yml`/`staging.yml`/`release.yml` (`tool/check_deferred_split.py`) falliscono se una stringa admin ricompare in `main.dart.js` o nei part di `Protected`, e anche se la build è `--wasm` (esiste `main.dart.wasm`): quella build produce comunque un `main.dart.js` di fallback con i suoi part, ma su Chromium carica il `.wasm` con tutto dentro. Il renderer è CanvasKit (da gstatic). Il loader di Flutter 3.41 usa wasm solo su Chromium: Safari/iOS e Firefox prendono comunque `main.dart.js`. Tornando a `--wasm` (vedi costi e alternative in `docs/PERF_AVVIO.md`, Fase 4) lo split resterebbe utile solo a quei browser: adatta il gate, riaggiungi `main.dart.wasm`/`.mjs` al `no-cache` di `web/.htaccess` se li togli.
