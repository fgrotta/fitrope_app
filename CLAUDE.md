...
- **Alias OneSignal prima dell'invio**: `include_aliases: {external_id: [...]}` fallisce silenziosamente (200 OK, `recipients: 0`, nessun errore) se l'utente non ha mai fatto login — l'alias viene creato solo al login self-service. Ogni nuovo invio mirato a un utente specifico deve richiamare `ensureOneSignalUser(userId, email)` (idempotente) subito prima dell'invio, non solo al login: pattern già applicato in `scheduleTrialReminder` e `notifyWaitlistUsers`. Caso tipico da non riprodurre: utente creato da Admin (walk-in) e mai loggato.
