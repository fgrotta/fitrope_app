...
> I piani dettagliati vivevano in `.context/` (gitignored, effimero per workspace):
> i file citati in origine non esistono più. Il contesto storico è in questo
> documento e in `docs/PIANO_STAGING.md` / `docs/AMBIENTI_DI_TEST.md`.
...
| Commit | PR | Contenuto |
|---|---|---|
| `7de7a50` | **PR1** | `Sale`, tag/registry tipologie (`CourseType`), `Course.sala` (+ serializzazione, `copyWith`), selettore sala in CourseManagementPage/RecurringCoursePage (la sala e diventata visibile in lettura sulla CourseCard solo con il redesign del calendario, vedi sotto) |
...
| *(PR6)* | **PR6** | `firestore.rules` nel repo + blocco `firestore` in firebase.json (l'emulatore le carica: QA manuale realistico). **Lockdown field-level basato su `diff().affectedKeys()`**: self → whitelist campi profilo (isActive solo auto-disattivazione); Admin → anagrafica gestionale MA MAI i server-owned (courses/waitlistCourses/activeSubscriptions/enrollmentConsumption/cancelledEnrollments/uid/email/createdAt); Trainer → solo anagrafica di utenti User; registrazione self vincolata al profilo-prova standard (no auto-crediti/ruoli); corsi: create Admin/Trainer con contatori azzerati, update senza subscribed/waitlist (Trainer solo propri), delete solo callable; subscriptions read-self/write-server-only. DECISIONE: rules field-level + **updateUser DIFF-BASED** (invia solo i campi cambiati vs original, confronto per-giorno su date/set sui tag) INVECE della callable adminUpdateUser — il payload completo includeva chiavi aggiunte/Timestamp rinormalizzati in `affectedKeys` e avrebbe NEGATO il salvataggio del proprio profilo (blocker del 1° gate, risolto); `users.role` ora affidabile (nessuna auto-promozione). create-self vincolata con `hasOnly` (no tag-premium/trial-esteso/entrateSettimanali auto-grantabili); Trainer crea solo User; corsi: Trainer create solo propri + no riassegnazione trainerId; regolamentoAccettatoIl protetto. registration.dart scrive la shape canonica completa. Client: rimosso cancelledEnrollments da updateUser, rimosso deleteUser.dart. `updateUser` diff-based estratto in `buildUserUpdateDiff` PURO + `test/update_user_test.dart`; fix robustezza: campo entrate svuotato non azzera il credito. DECISIONE documentata: la create manuale dei Trainer NON vincola tag/entrate (flusso gestionale legittimo; i doc manuali hanno id≠auth-uid → non loggabili). **Categoria D consegnata**: suite @firebase/rules-unit-testing (23 test: payload REALI su doc registration-shaped legacy, escalation negati, regolamento self-only) |
| *(recupero)* | **Recupero in giornata** | La penalità per disdetta tardiva non è più definitiva: una lezione persa è ASSORBITA, uno a uno, da un'iscrizione attiva della **stessa giornata e tipologia** (slot consumati in un giorno = `max(attive, persi)`). Regola **derivata**, senza stato nuovo: nessun token, nessuna scadenza da schedulare, nessun cron — scadenza a mezzanotte, ritorno del recupero se si disdice il rimpiazzo, due disdette = due recuperi sono tutte conseguenze del conteggio. Netting in `countWeeklyEntries` (il candidato è incluso e poi sottratto, perché il gate `weeklyUsed >= limite` non lo conta) + nuova `countRecoverableEntries` per la decisione di consumo dei piani a ingressi (`consume: NONE`, blocco `NO_ENTRIES` sollevato). Unico campo nuovo: `cancelledEnrollments.lostKind` (`ENTRY`/`WEEKLY_SLOT`, assente ⇒ `WEEKLY_SLOT`), che **sostituisce** il workaround `trackCancelled && !consumedAnEntry` — `decideRefund` ora riceve `consumedEntry` e risolve `trackCancelled`+`lostKind`. Nessuna modifica a `firestore.rules` (`cancelledEnrollments` era già server-owned). UI: copy del dialog con il numero di corsi di recupero ancora disponibili oggi, banner in HomePage, label admin distinta per tipo di perdita |
| *(dashboard)* | **Dashboard admin senza legacy** | `AdminDashboardPage` legge solo il modello V2: rimosse "Per tipologia legacy", "Distribuzione per piano", "Distribuzione legacy per tipologia" e i grafici per tag. Card Utenti: "Per durata abbonamento" (Prova → Mensile → Trimestrale → Semestrale → Annuale, voci a zero incluse e non tappabili) e "Abbonamenti per tipologia" in ordine fisso, entrambe contando **soci** (numero = righe del drawer). Card Abbonamenti: solo KPI scadenze 30 gg e ingressi medi residui sui soli abbonamenti vivi a ingressi (nessun ramo `entrateDisponibili`). Helper puri in `lib/utils/subscription_duration.dart`, che sostituisce `subscription_ordering.dart`; test `test/subscription_duration_test.dart`. |

Verifica (conteggi correnti, fonte: le suite stesse): `flutter analyze` pulito, **flutter test 400** (recupero: +21, calendario #9: +53), **functions 301 jest** (recupero: +29) + **35 integrazione su emulatore** (recupero: +2; callable + rules) + `tsc` ok. Gate multi-agent per ogni PR (PR6: 4 passate — blocker payload-completo → fix updateUser diff-based + unit test; major rules create chiusi/documentati; 4° gate PASS pulito).
...
### Staging (opzione C) — ✅ FATTO

Realizzato e automatizzato: progetto `fit-rope-staging` su Blaze, deploy a ogni
push su `develop` via `staging.yml` (dettagli in `CLAUDE.md` e `DEPLOYMENT.md`),
alias in `.firebaserc`, `--dart-define=APP_ENV=staging` con
`lib/firebase_options_staging.dart` (scritto a mano, non da flutterfire), seed
sintetico (MAI dati reali: GDPR). `ONESIGNAL_APP_ID` è parametrizzato lato
Functions (env); resta hardcoded lato client in `lib/main.dart` con l'appId di
produzione e NON esiste una seconda app OneSignal — vedi il TODO "OneSignal
web" in `CLAUDE.md`.

### PR7 — UI polish + docs ✅ FATTO
- HomePage: card multi-abbonamento (residui/frequenza/scadenza per abbonamento) con fallback legacy; label per il nuovo modello (`subscription_labels.dart`, famiglie + "illimitato" + status "Scade oggi"/singolare/**"Esaurito"** per ENTRIES a residui zero); lista abbonamenti in UserDetailPage; **AdminDashboardPage: distribuzione "Per famiglia abbonamento"** (da `activeSubscriptions` vivi, dedup per famiglia) accanto a quella legacy per tipologia (quest'ultima poi rimossa, vedi riga *(dashboard)*); `README_ISCRIZIONI.md` aggiornato col flusso server-side + display.
...
- Node: `engines` in `functions/package.json` è `>=22 <25`. Il runtime Functions è 22, quindi i job che lo esercitano davvero (`functions-integration`, deploy) girano su 22, mentre i job unit usano 24 per compatibilità tooling. In locale serve almeno 22 (sul Mac di sviluppo i binari in `/usr/local/bin` puntano a versioni miste nvm/Homebrew: verificare con `node --version` prima di lavorare su functions).
