...
void main() {
  group('CourseUnsubscribeHelper Tests', () {
    ...
    setUp(() {
      ...
      courseSoon = Course(
        ...
        endDate:
            Timestamp.fromDate(DateTime.now().add(const Duration(hours: 7))),
        capacity: 20,
        subscribed: 5,
      );
      ...
      courseExact = Course(
        ...
        startDate: Timestamp.fromDate(
            DateTime.now().add(const Duration(hours: 8, seconds: 30))),
        endDate: Timestamp.fromDate(
            DateTime.now().add(const Duration(hours: 9, seconds: 30))),
        capacity: 20,
        subscribed: 5,
      );
      ...
      testUserPacchetto = FitropeUser(
        ...
        courses: [
          ...
          'test-course-exact',
          'test-course-past',
          'test-course-almost-8',
          'test-course-just-over-8'
        ],
    ...
    group('canUnsubscribe', () {
      ...
      test(
          'should return true for Pacchetto Entrate with exactly 8 hours remaining',
          () {
        final result = CourseUnsubscribeHelper.canUnsubscribe(
            courseExact, testUserPacchetto);
      ...
      test(
          'should return true for Pacchetto Entrate with course already started',
          () {
        final result = CourseUnsubscribeHelper.canUnsubscribe(
            coursePast, testUserPacchetto);
      ...
      test(
          'should return true for Abbonamento without confirmation requirement',
          () {
        final result = CourseUnsubscribeHelper.canUnsubscribe(
            testCourse, testUserAbbonamento);
    ...
    group('Edge Cases', () {
      ...
      test('should handle course starting in exactly 7 hours and 59 minutes',
        ...
        final courseAlmost8 = Course(
          ...
          startDate: Timestamp.fromDate(
              DateTime.now().add(const Duration(hours: 7, minutes: 59))),
          endDate: Timestamp.fromDate(
              DateTime.now().add(const Duration(hours: 8, minutes: 59))),
          capacity: 20,
          subscribed: 5,
        );

        final result = CourseUnsubscribeHelper.canUnsubscribe(
            courseAlmost8, testUserPacchetto);
      ...
      test('should handle course starting in exactly 8 hours and 1 minute', () {
        final courseJustOver8 = Course(
          ...
          startDate: Timestamp.fromDate(
              DateTime.now().add(const Duration(hours: 8, minutes: 5))),
          endDate: Timestamp.fromDate(
              DateTime.now().add(const Duration(hours: 9, minutes: 5))),
          capacity: 20,
          subscribed: 5,
        );

        final result = CourseUnsubscribeHelper.canUnsubscribe(
            courseJustOver8, testUserPacchetto);
  ...
  group('canUnsubscribe — modello multi-abbonamento', () {
    ...
    test('abbonamento ENTRIES: finestra 8h (conferma entro, rimborso oltre)',
      ...
      final outside =
          CourseUnsubscribeHelper.canUnsubscribe(courseInHours(10), user);
      expect(outside['requiresConfirmation'], false);
      expect(outside['isPacchettoEntrate'], true);
      expect(
          outside['message'], 'Disiscrizione: il credito ti sarà rimborsato');
    ...
    test(
        'snapshot con SOLE voci scadute: torna al modello legacy (finestra 8h)',
        () {
